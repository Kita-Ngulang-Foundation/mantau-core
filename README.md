...
after "a fall was detected" â€” the event shape, who gets told, how, and how
that's measured â€” lives here once, so the comparison between the two
scenarios is honest.
...
| Module | What it owns |
|---|---|
| `contracts` | `FallEvent`, `CameraRef`, `Envelope` (signed agentâ†’server wire format), the reachability error taxonomy. The vocabulary â€” extend here first, never redeclare locally. |
| `detection` | `Detector` protocol, `NullDetector` (always-empty, or fires on a fixed cadence for wiring tests), `MediapipeDetector` â€” **the only module that imports `mantau.*`** (the CV package), gated behind the `detection` extra. |
| `notify` | `Fanout` â†’ `Alert` â†’ channels (`push` built to completion, `telegram` explicitly temporary, `console` for dev) â†’ `DeliveryTracker` â†’ `AckService` â†’ `EscalationChain`/`EscalationPolicy`. |
| `buffer` | `DurableSpool` â€” a SQLite-backed, TTL-evicting queue so a brief outage doesn't silently lose an event. |
| `resilience` | `BackoffPolicy` (full-jitter exponential backoff) and `Supervisor` (restart a task with backoff, bounded, observable). |
| `telemetry` | `LatencyTrace` (`captured â†’ detected â†’ queued â†’ sent â†’ delivered â†’ acked`, the number the whole project is judged on) and a tiny `MetricsRegistry`. |
| `config` | `CoreSettings` â€” a `pydantic-settings` base each backend subclasses with its own fields. |
...
Optional extras:
- `.[push]` â€” pulls in `google-auth`, needed only to actually send FCM push (`ServiceAccountCredentials`). Importing `mantau_core.notify` never requires it.
- `.[detection]` â€” pulls in `mantau` (the CV package) at a pinned SHA, needed only to construct `MediapipeDetector`.
...
All tests run offline â€” HTTP channels (FCM, Telegram) are tested against
...
a streaming entrypoint â€” `mantau.api.streaming.StreamingDetector`, taking
...
Telegram is explicitly temporary â€” demo insurance in case FCM misbehaves on
...
Docker Compose â€” `mantau-core` never opens a database connection or a
